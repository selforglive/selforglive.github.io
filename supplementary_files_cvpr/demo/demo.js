...
function createDemo(divId, modelsSet) {
  ...
  async function initLegend(models) {
    ...
    models.model_names.forEach((name, idx, _) => {
      ...
      console.log(x, y);
      console.log(w, h);
      texture.style.background = "url('demo/demo/sprites.png') " + x + "% " + y + "%";
  ...
  function updateUI() {
    ...
    $("#origtex").style.background = "url('demo/demo/dtd_sprites.png') " + x + "% " + y + "%";
  ...
  async function updateCA() {
    ...
    const models = MODELS;//await r.json();
